MEMERANK - a completly serverless meme ranking website for uploding and rating diffrent memes online
...
Browser → API Gateway → getUploadUrl(lambda) → S3 presigned URL

Browser → S3 (direct upload using presigned URL)

Browser → API Gateway → confirmUpload(lambda) → DynamoDB

Ranking flow:

Browser → API Gateway → getRandomMeme(lambda) → DynamoDB (filters already-rated memes)

Browser → API Gateway → submitRating(lambda) → DynamoDB (updates score + records session)


Scoreboard flow:

Browser → API Gateway → getScoreboard(lambda) → DynamoDB (sorted by average score)
